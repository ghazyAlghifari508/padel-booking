...
export function CourtCard({ court }: { court: Court }) {
  return (
    <Link href={`/courts/${court.id}`} className="group block rounded-2xl border border-border bg-surface">
      <div className="relative aspect-3/4 overflow-hidden rounded-t-2xl border-b border-border bg-muted-surface">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4 border-b border-border pb-4">
          ...
          <p className="text-right text-[13px] uppercase tracking-[0.08em] text-foreground">{formatIDR(court.pricePerHour)}/hr</p>
        </div>
        <p className="mt-4 min-h-12 text-[13px] leading-relaxed text-muted">{court.description}</p>
        <span className="mt-5 block rounded-full bg-primary px-5 py-3 text-center text-[13px] uppercase tracking-[0.08em] text-foreground">View slots</span>
      </div>
    </Link>
  );
}
