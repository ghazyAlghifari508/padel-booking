  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      <div className="flex items-start justify-between gap-4 border-b border-border pb-4">
        <div><p className="text-[13px] uppercase tracking-[0.08em] text-muted">Booking #{booking.id}</p><h3 className="mt-2 text-[26px] font-normal leading-tight tracking-[-0.02em] text-foreground">{booking.courtName}</h3></div>
      ...
      <div className="mt-5 flex items-center justify-between gap-4 border-t border-border pt-4"><span className="text-[22px] tracking-[-0.02em] text-foreground">{formatIDR(booking.totalPrice)}</span>{action}</div>
    </div>
  );
}
