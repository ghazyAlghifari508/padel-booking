              ...
              {list.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-muted-surface/50">
                    <td className="px-4 py-3 font-mono text-xs font-normal text-foreground">{p.reference}</td>
                    <td className="px-4 py-3"><p className="font-normal text-foreground">#{p.bookingId}</p><p className="text-xs font-medium text-muted">{b?.userName ?? "—"}</p></td>
                    <td className="px-4 py-3 font-normal tabular-nums text-foreground">{formatIDR(p.amount)}{mismatch && <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-status-cancelled-soft px-2 py-0.5 text-xs text-status-cancelled"><AlertTriangle className="h-3 w-3" /> mismatch</span>}</td>
