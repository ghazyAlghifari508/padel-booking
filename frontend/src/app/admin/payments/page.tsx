              ...
              {list.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-foreground/[0.02]">
                    ...
                    <td className="px-4 py-3"><p className="font-medium text-foreground">#{p.bookingId}</p><p className="text-xs text-muted">{b?.userName ?? "—"}</p></td>
                    <td className="px-4 py-3 font-medium tabular-nums text-foreground">{formatIDR(p.amount)}{mismatch && <span className="ml-2 inline-flex items-center gap-1 rounded-md bg-red-50 px-2 py-0.5 text-xs font-medium text-red-600"><AlertTriangle className="h-3 w-3" /> mismatch</span>}</td>
