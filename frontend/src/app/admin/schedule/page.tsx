  ...
  return (
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="sky-card rounded-[24px] p-5"><h2 className="flex items-center gap-2 font-extrabold text-foreground"><Clock className="h-5 w-5 text-primary-hover" /> Operating hours</h2><div className="mt-4 space-y-2">{hours.map((h) => <div key={h.dayOfWeek} className="flex items-center justify-between rounded-[16px] border border-border bg-primary-soft px-3 py-2.5 text-sm"><span className="font-extrabold text-foreground">{DAYS[h.dayOfWeek]}</span>{h.closed ? <span className="font-extrabold text-status-cancelled">Closed</span> : <span className="tabular-nums font-bold text-muted">{h.openTime} – {h.closeTime}</span>}</div>)}</div></div>
        <div className="sky-card rounded-[24px] p-5"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 font-extrabold text-foreground"><Ban className="h-5 w-5 text-status-cancelled" /> Blocked time</h2><Button size="sm" onClick={() => setAdding(true)}><Plus className="h-4 w-4" /> Add</Button></div><div className="mt-4 space-y-2">{blocked.filter((b) => b.courtId === courtId).length === 0 ? <p className="rounded-[16px] border border-dashed border-border bg-primary-soft px-3 py-4 text-center text-sm font-medium text-muted">No blocked time for this court.</p> : blocked.filter((b) => b.courtId === courtId).map((b) => <div key={b.id} className="flex items-center justify-between rounded-[16px] border border-dashed border-status-cancelled/30 bg-status-cancelled-soft/40 px-3 py-2.5 text-sm"><div><p className="font-extrabold text-foreground">{formatDate(b.date)} · {b.startTime}–{b.endTime}</p><p className="text-xs font-medium text-muted">{b.reason}</p></div><button onClick={() => setBlocked((p) => p.filter((x) => x.id !== b.id))} aria-label="Remove" className="cursor-pointer rounded-full p-2 text-muted hover:bg-status-cancelled-soft hover:text-status-cancelled"><Trash2 className="h-4 w-4" /></button></div>)}</div></div>
      </div>
