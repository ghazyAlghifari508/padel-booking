...
export default function AdminSchedulePage() {
  ...
  const active = useMemo(() => (courts ?? []).filter((c) => c.status === "active"), [courts]);
  const [courtId, setCourtId] = useState(0);
  const effectiveCourtId = courtId || active[0]?.id || 0;
  const { data: hours } = useApi(() => effectiveCourtId ? api.admin.operatingHours(effectiveCourtId) : Promise.resolve([]), [effectiveCourtId]);
  ...
  const addBlock = async () => {
    await api.admin.createBlocked({ courtId: effectiveCourtId, ...draft });
    setAdding(false);
  ...
  return (
    <div>
      ...
      <div className="mb-5 w-full sm:w-80"><Field label="Select court" htmlFor="court"><Select id="court" value={effectiveCourtId} onChange={(e) => setCourtId(Number(e.target.value))}>{active.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</Select></Field></div>
